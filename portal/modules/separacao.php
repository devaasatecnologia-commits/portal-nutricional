<?php
$pageTitle = 'Separação Nutricional | Coletor Oficial';
$moduleJs = 'separacao.js';
$version = time();
...
<link rel="stylesheet" href="' . $appBase . '/portal/assets/css/separacao.css?v=' . $version . '">
';
...
<div class="mobile-toolbar block lg:hidden fixed top-0 left-0 right-0 z-50 shadow-lg">
    <div class="flex items-center justify-between px-4 py-3">
        ...
        <div class="text-center">
            <span class="text-sm font-bold modulo-nome">SEPARAÇÃO</span>
        </div>
...
<div class="modulo-container max-w-full mx-auto px-4 lg:px-6 py-4">
    ...
    <div class="glass rounded-3xl p-5 mb-6 flex justify-between items-center shadow-sm">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-lg font-bold text-slate-800 leading-none">SEPARAÇÃO</h2>
                <span id="label-embarque" class="text-xs text-slate-400 font-medium">CONFERÊNCIA DE PEDIDOS</span>
            </div>
        </div>
    ...
    <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 mb-6">
        <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Selecione o Embarque</label>
        
        <div id="btnAbrirSelecao" onclick="toggleMenuEmbarques()" class="w-full p-4 rounded-xl border-2 border-slate-200 font-bold bg-slate-50 flex justify-between items-center text-slate-700 cursor-pointer hover:border-emerald-500 transition-colors">
    ...
    <div id="areaOperacional" style="display:none;">
        ...
        <div class="grid grid-cols-3 gap-3 mb-6">
            <div class="bg-white p-3 rounded-2xl border border-slate-100 text-center shadow-sm">
                <p class="text-[10px] uppercase font-bold text-slate-400 mb-1">Separados</p>
                <span id="resumo-total-itens" class="text-sm font-bold text-slate-700">0/0</span>
            </div>
            <div class="bg-white p-3 rounded-2xl border border-slate-100 text-center shadow-sm">
                <p class="text-[10px] uppercase font-bold text-slate-400 mb-1">Peso Total</p>
        ...
        <div class="flex gap-2 mb-6">
            <button id="btnASC" class="flex-1 py-3 rounded-xl font-bold text-sm bg-slate-700 text-white shadow-md" onclick="alterarOrdem('ASC')">
                <i class="fa-solid fa-arrow-down-short-wide"></i> PADRÃO
            </button>
            <button id="btnDESC" class="flex-1 py-3 rounded-xl font-bold text-sm bg-slate-200 text-slate-600 hover:bg-slate-300 transition-colors" onclick="alterarOrdem('DESC')">
                <i class="fa-solid fa-arrow-up-wide-short"></i> INVERTER
            </button>
        </div>

        <div id="reader" style="display:none;" class="rounded-2xl overflow-hidden border-4 border-emerald-600 mb-4 shadow-2xl bg-black"></div>
        ...
        <div class="sticky top-20 z-40 pb-4">
            <div class="relative group">
                ...
                placeholder="Bipe para separar..." 
                inputmode="numeric" 
                autocomplete="off"
                readonly 
                onclick="this.removeAttribute('readonly'); this.focus();">
                <i class="fa-solid fa-expand absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-emerald-600 transition-colors"></i>
                <button onclick="toggleCamera()" class="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 bg-slate-100 text-slate-600 rounded-xl hover:bg-emerald-600 hover:text-white transition-all flex items-center justify-center">
        ...
        <div id="container-finalizar" style="display:none;" class="mb-6">
            <button onclick="finalizarSeparacao()" class="w-full bg-emerald-600 text-white py-4 rounded-2xl font-bold shadow-xl hover:bg-emerald-700 transition-all flex items-center justify-center gap-3 active:scale-95">
                <i class="fa-solid fa-check-circle text-xl"></i> FINALIZAR SEPARAÇÃO
            </button>
        </div>
